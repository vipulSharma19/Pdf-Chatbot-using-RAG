...
class VectorStore:
    VECTOR_STORE_PATH = "C:/Users/nirde/Downloads/Multi-PDF-Chatbot-using-Gemini-master/Multi-PDF-Chatbot-using-Gemini-master/vectors"  # Define a path for the vector store

    @staticmethod
    def delete_existing():
        """Delete the existing vector store if it exists."""
        if os.path.exists(VectorStore.VECTOR_STORE_PATH):
            print(f"Deleting existing vector store at {VectorStore.VECTOR_STORE_PATH}")
            shutil.rmtree(VectorStore.VECTOR_STORE_PATH)

    @staticmethod
    def create(documents: List[Document]) -> FAISS:
        print("Creating new vector store...")
        VectorStore.delete_existing()  # Delete existing store before creating a new one
        embeddings = GoogleGenerativeAIEmbeddings(model="models/embedding-001", google_api_key=GOOGLE_API_KEY)
        vector_store = FAISS.from_documents(documents, embeddings)
        
        # Save the new vector store
        os.makedirs(VectorStore.VECTOR_STORE_PATH, exist_ok=True)
        vector_store.save_local(VectorStore.VECTOR_STORE_PATH)
        
        print(f"Vector store created and saved at {VectorStore.VECTOR_STORE_PATH}")
        return vector_store

    @staticmethod
    def load() -> FAISS:
        """Load the vector store if it exists, otherwise return None."""
        if os.path.exists(VectorStore.VECTOR_STORE_PATH):
            embeddings = GoogleGenerativeAIEmbeddings(model="models/embedding-001", google_api_key=GOOGLE_API_KEY)
            return FAISS.load_local(VectorStore.VECTOR_STORE_PATH, embeddings)
        return None
...
def main():
    try:
        ...
        print(f"Loading PDFs from: {PDF_PATHS}")
        ...
        # Delete existing vector store and create a new one
